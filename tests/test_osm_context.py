...
def test_parse_osm_context_keeps_nearby_roads_and_named_places() -> None:
    document = {
        "elements": [
    ...
    context = parse_osm_context(document, _main_route(), 180.0)
...
def test_fetch_osm_context_uses_injected_transport() -> None:
    ...
    assert "(around:180," in captured[0]
